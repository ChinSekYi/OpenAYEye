...
def create_db(user = "root", 
        password = "msql1234", 
        server= "localhost", 
        database = "mock"
    ):
    
    SQLALCHEMY_DATABASE_URL = "mysql+pymysql://{}:{}@{}/{}".format(user, password, server, database)
...
def getEntries(columns, table):
    ...
    query_string = sqlalchemy.text('''SELECT {} FROM {} LIMIT 100'''.format(col_string, table))
...
@app.get('/getContacts')
def getContacts(columns = ['id', 'name', 'email', 'age', 'phone', 'address', 'city', 'zipCode', 'registrarId'], table = 'users'):
    return getEntries(columns, table)

@app.get('/getInvoice')
def getInvoice(columns = ['id', 'name', 'email', 'phone', 'cost', 'date'], table = 'invoice'):
    db = engine.connect()
