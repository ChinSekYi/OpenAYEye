...
class ModelTrainerConfig:
    ...
    trained_fixed_deposits_model_file_path = os.path.join(
        "src/artifacts", "reco_sys_fixed_deposits_model.pkl"
    )
    trained_loan_model_file_path = os.path.join("src/artifacts", "reco_sys_loan_model.pkl")

    trained_credit_card_debit_card_file_path = os.path.join(
        "src/artifacts", "reco_sys_credit_card_debit_card_model.pkl"
    )
    trained_account_model_file_path = os.path.join(
        "src/artifacts", "reco_sys_account_model.pkl"
    )

    trained_fixed_deposits_model_file_path = os.path.join(
        "src/artifacts", "reco_sys_fixed_deposits_model.pkl"
    )
    trained_loan_model_file_path = os.path.join(
        "src/artifacts", "reco_sys_loan_model.pkl"
    )
    trained_credit_card_debit_card_file_path = os.path.join(
        "src/artifacts", "reco_sys_credit_card_debit_card_model.pkl"
    )
...
class ModelTrainer:
    ...
    def train_models(self, X_train, y_train):
        ...
        random_search = RandomizedSearchCV(
            lr_model, param_dist, n_iter=8, cv=skf, scoring="f1_macro", n_jobs=-1
        )
        ...
        for i in tqdm(range(y_train.shape[1]), desc="Training Models", unit="model"):
            ...
            logging.info(
                f"Best parameters for {y_train.columns[i]}: {random_search.best_params_}"
            )
    ...
    def evaluate_model(self, model, X_test, y_test, column_name):
        ...
        classification = classification_report(
            y_test[column_name], y_pred, output_dict=True
        )
...
if __name__ == "__main__":
    ...
    try:
        # Load your dataset
        df_train = pd.read_csv("src/artifacts/reco_sys_train_data.csv")
        df_test = pd.read_csv("src/artifacts/reco_sys_test_data.csv")

        # For covid dataset
        """ 
        df_train = pd.read_csv("src/artifacts/clean_train_reco_covid.csv")
        df_test = pd.read_csv("src/artifacts/clean_test_reco_covid.csv")
        """
