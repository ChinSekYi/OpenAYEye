    ...
    def initiate_model_trainer(self, df):
        ...
        try:
            ...
            model = RegressorChain(
                base_model, order=[0, 1, 2]
            )  # Predict clicks, then leads, then orders
            ...
            logging.info(
                f"Saving the trained model to {self.model_trainer_config.trained_model_file_path}"
            )
            ...
            joblib.dump(
                encoder, self.model_trainer_config.encoder_path
            )  # Save the encoder
