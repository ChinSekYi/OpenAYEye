...
class RecoSysCustomData:
    ...
    join_channel_mapping = {
        "Referral Program": "KHE",
        "Customer Service": "KHD",
        "Personal Banker": "KFA",
        "Corporate Website": "KHC",
        "Mobile Advertising": "KFC",
        "Online Chat": "RED",
        "Third-Party Website": "KDH",
        "Community Events": "KHK",
        "Online Campaigns": "KEH",
        "Customer Rewards Program": "KBG",
        "Referral from Family/Friends": "KHL",
        "Bank Promotions": "KGC",
        "In-Person Events": "KHF",
        "Loyalty Program": "KFK",
        "Credit Union Partnership": "KHN",
        "University Partnership": "KHA",
        "Trade Show": "KHM",
        "Sales Representative": "KGX",
        "Email Marketing": "KFD",
        "Social Ads": "KFG",
        "Corporate Outreach": "KCC",
        "Government Program": "KFJ",
        "Television Ads": "KFL",
        "Radio Ads": "KFU",
        "Newspaper Ads": "KFS",
        "Magazine Ads": "KAA",
        "Neighborhood Event": "KFP",
        "Student Program": "KFN",
        "Public Billboard": "KGV",
        "Community Board": "KGY",
        "Targeted Ads": "KFF",
        "Digital Influencers": "KDI",
        "Local Partnerships": "KLP",
        "Affiliate Marketing": "KAM",
        "Content Marketing": "KCM",
        "Online Forums": "KOF",
        "Corporate Sponsorships": "KCS",
        "Pop-Up Events": "KPE",
        "Charity Events": "KCE",
        "Public Seminars": "KPS",
        "Employee Advocacy": "KEA",
        "Co-Branding": "KCB",
        "Sponsored Content": "KSC",
    }
    customer_segment_mapping = {
        "VIP": "01 - TOP",
        "Private": "02 - PARTICULARES",
        "University": "03 - UNIVERSITARIO",
    }
    region_mapping = {
        "East": "EAST",
        "North": "NORTH",
    ...
    def __init__(self, **kwargs):
        ...
        self.gender = self.gender_mapping.get(
            kwargs.get("gender"), kwargs.get("gender")
        )
        self.gross_income = kwargs.get("gross_income")
        self.customer_segment = self.join_channel_mapping.get(
            kwargs.get("customer_segment"), kwargs.get("customer_segment")
        )
        ...
        self.primary_customer_status = self.primary_customer_status_mapping.get(
            kwargs.get("primary_customer_status"), kwargs.get("primary_customer_status")
        )
        self.new_customer_index = kwargs.get("new_customer_index")
        month = kwargs.get("customer_type_start_month")
        self.customer_type_start_month = self.customer_type_start_month.get(
            month, "Jan"
        )
        self.country_residence = self.country_residence_mapping.get(
            kwargs.get("country_residence"), kwargs.get("country_residence")
        )
        self.region = self.join_channel_mapping.get(
            kwargs.get("region"), kwargs.get("region")
        )
        self.join_channel = self.join_channel_mapping.get(
            kwargs.get("join_channel"), kwargs.get("join_channel")
        )
        self.deceased_index = self.deceased_index_mapping.get(
            kwargs.get("deceased_index"), kwargs.get("deceased_index")
        )
        self.foreigner_index = self.foreigner_index_mapping.get(
            kwargs.get("foreigner_index"), kwargs.get("foreigner_index")
        )
        self.residence_index = self.residence_index_mapping.get(
            kwargs.get("residence_index"), kwargs.get("residence_index")
        )
        self.customer_relation_type = self.customer_relation_type_mapping.get(
            kwargs.get("customer_relation_type"), kwargs.get("customer_relation_type")
        )
        new_customer_status = kwargs.get("new_customer_index")
        self.new_customer_index = self.new_customer_index_mapping.get(
    ...
    def get_data_as_dataframe(self):
        try:
            ...
            custom_data_input["primary_customer_status"] = [
                self.primary_customer_status
            ]
            ...
            custom_data_input["customer_type_start_month"] = [
                self.customer_type_start_month
            ]
            ...
            country_values = [
                ...
                "NI",
                "AL",
            ]  # Removed 'CA', 'IE' bc not in training set

            for country in country_values:
                custom_data_input[f"country_residence_{country}"] = [
                    1 if self.country_residence == country else 0
                ]
            ...
            for channel in join_channel_values:
                custom_data_input[f"join_channel_{channel}"] = [
                    1 if self.join_channel == channel else 0
                ]
