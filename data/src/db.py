...
from data import campaign, engagement, engine, santender
...
def to_snakecase(df):
    snakecase = {
        i: re.sub(r"[,.;@#?!&$]+\ *", "", i.strip()).replace(" ", "_") for i in df
    }
...
def get_users(path="data/users.csv", date_format="%Y-%m"):
    ...
    users["user"] = (
        users["user"].astype(str).str.pad(width=4, side="left", fillchar="0")
    )
    users["birth_year"] = (
        users["birth_year"].astype(str) + "-" + users["birth_month"].astype(str)
    )
    users["birth_year"] = pd.to_datetime(
        users["birth_year"], format=date_format
    ).astype("datetime64[ns]")

    users = users.drop(columns=["birth_month"])
    users = users.rename(
        columns={"user": "customer_id", "birth_year": "birth_year_month"}
    )
    users["gender"] = users["gender"].astype("category")
    for i in ["fixed_deposits", "credit_card_debit_card", "account"]:
        est = get_est(santender, y_col=i)
        test = users[["current_age", "yearly_income"]].rename(
            columns={"current_age": "age", "yearly_income": "gross_income"}
        )
        users[i] = est.predict(test)
    users["loan"] = [
        random.choices([0, 1], weights=[1 - 0.015, 0.015])[0] for _ in range(len(users))
    ]
    users = users.rename(
        columns={"fixed_deposits": "deposits", "credit_card_debit_card": "cards"}
    )
...
def get_creditcards(path="data/credit_cards.csv", date_format="%m/%Y"):
    ...
    credit_cards["user"] = (
        credit_cards["user"].astype(str).str.pad(width=4, side="left", fillchar="0")
    )
    credit_cards["expires"] = pd.to_datetime(
        credit_cards["expires"], format=date_format
    )
    credit_cards["acct_open_date"] = pd.to_datetime(
        credit_cards["expires"], format=date_format
    )
    credit_cards["year_pin_last_changed"] = pd.to_datetime(
        credit_cards["year_pin_last_changed"], format="%Y"
    )
    return credit_cards
...
def get_transactions(path="data/transactions.csv", date_format="%m/%Y"):
    ...
    transactions["user"] = (
        transactions["user"].astype(str).str.pad(width=4, side="left", fillchar="0")
    )
    ...
    card_no = (
        transactions.merge(cc_no, how="inner", on=["user", "card_index"])["card_number"]
        .astype(str)
...
def get_churn(users, start_date=datetime(2023, 1, 1), end_date=datetime(2024, 10, 31)):
