...
campaign_ids = [
    f"{str(i).zfill(4)}" for i in range(1, len(campaign))
]  # 50 campaign IDs
...
for _, eng in engagement.iterrows():
    ...
    probs = action_type_probability(
        camp["target_segment"], camp["budget"], camp["channel"], camp["goal"]
    )
    # Choose action_type based on calculated probabilities
    action_type = random.choices(list(probs.keys()), weights=list(probs.values()), k=1)[
        0
    ]
    updated_action_types.append(action_type)

engagement["action_type"] = updated_action_types
engagement["conversion_value"] = [
    seasonal_conversion_value(action) for action in engagement["action_type"]
]
