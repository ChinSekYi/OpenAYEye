...
def get_Products(
    ...
    def create_additional_columns(df):
        ...
        if credit_card_debit_card_col not in df.columns:
            ...
            df[credit_card_debit_card_col] = (
                df[credit_card_columns].any(axis=1).astype(int)
            )
    ...
    santender["last_primary_customer_date"] = pd.to_datetime(
        santender["last_primary_customer_date"]
    )
    ...
    santender["age"] = pd.to_numeric(
        santender["age"].replace(" NA", None), errors="coerce"
    )
    med_age = santender["age"].median()
    santender["age"] = santender["age"].fillna(med_age).astype(int)
    santender["gross_income"] = pd.to_numeric(
        santender["gross_income"].replace(" NA", None), errors="coerce"
    )
